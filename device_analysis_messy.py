...
# dir_path = "\\InGaAs contacts\\Janis\\2019-11-10-Ge-MoS2"
# dir_path = "\\InGaAs contacts\\Janis\\2019-10-31-Ge-MoS2"
...
# dir_path = "\\InGaAs contacts\\Janis\\2020-01-29-ME2"
...
#        fig,ax = plt.subplots(); ax.plot(self.gm); plt.show()
...
#        fig,ax = plt.subplots(); ax.plot(self.dgm); plt.show()
...
class IdVd:
    ...
    def __init__(self, inp):  # Argument is of type InputData
        ...
        self.Id = inp.id_vd[np.arange(num_vd),
    ...
    def plot_IdVd(self, ax, fig, Vgs=None):
        ...
        if flag == 1:
            ...
            ax.yaxis.set_ticks(np.arange(round(start/50)*50, (round(end/50)+1)*50, 50)) 
            #Increase tick frequency to 50
...
#        print(results.summary()) #Print the results of TLM fit
    ...
    def plot_TLM_fit(self, ax, fig):
        ...
        ax.set_xlim(left=0)  # Sets lower x-limit to 0
        sns.regplot(x=r'Channel Length [$\mu$m]', y=r'Total Resistance [k$\Omega$.$\mu$m]',\
                    data=self.TLM_fit, ci=95, truncate=False, ax=ax) 
        ...
        # ax.annotate(r'at $n_{2D}$ = ' + str(np.round(self.n2D[-1] / 1e13, 1)) + r'$\times10^{13} cm^{-2}$',
        #             xy=(.05, .75), xycoords='axes fraction', fontsize=8.5)
    ...
    def plot_mu_TLM(self, ax, fig, flag=None):
        ...
        if flag is None:
            ...
            ax.set(xlabel=r'Carrier Concentration [$10^{12}$ $cm^{-2}$]',
                   ylabel=r'Effective Mobility [$cm^2$$V^{-1}$$s^{-1}$]')  # , title=r'$R_C$ vs $n_{2D}$')
...
#        top_limit = np.maximum(2*mu_TLM_cut[int(np.shape(mu_TLM_cut)[0]/2)]\
#                                            ,mu_TLM_cut[0]+del_mu_TLM_cut[0])
        ...
        ax.annotate(r'at $n_{2D}$ = ' + str(np.round(self.n2D[-1] / 1e12, 1))\
                    + r'$\times10^{12} cm^{-2}$',
                    xy=(.05, .75), xycoords='axes fraction', fontsize=8.5)
        # ax.annotate(r'at $n_{2D}$ = ' + str(np.round(self.n2D[-1] / 1e13, 1))\
        #             + r'$\times10^{13} cm^{-2}$',
        #             xy=(.05, .75), xycoords='axes fraction', fontsize=8.5)
...
#        ax.set_ylim(bottom=0)  # Sets lower y-limit to 0
    ...
    def plot_IdVd(self, ax, fig, Vgs):
        ...
        ax.legend(legends, loc='best')
        ax.set(xlabel='Drain Voltage [V]',
               ylabel=r'Drain Current [$\mu A$/$\mu m$]')  # , title='Output Characteristics')
        #        ax.grid()
...
for i in device_index:
    ...
    fig, ((ax1, ax2, ax3)) = plt.subplots(1, 3) #When only 1 channel works
    # ax1.set_prop_cycle('color', Bold_6.mpl_colors)
    ...
    # tlm_set[i].plot_Rc(ax5, fig, 'n2D')  # Plotting Rc vs n2D
    # tlm_set[i].plot_mu_TLM(ax6, fig, 'n2D')  # Plotting Rc vs n2D
    # tlm_set[i].plot_TLM_fit(ax4, fig)  # Plotting Rc vs n2D
    plt.show()
    ...
    # for k in np.arange(tlm_set[i].count): #Testing Vt for each channel length
    #     print(tlm_set[i].idvg[k].Vt_lin)
    # tlm_set[i].idvd[0].plot_IdVd(ax3, fig)  # Plotting Id-Vd for 1st channel length
