...
def plot_TLM_fit(ax, fig, Rc, del_Rc, R_squared, TLM_fit):
    ...
    Rc_anno = np.round(Rc, 2)
    delRc_anno = np.round(del_Rc, 2)
    ax.annotate(r'$R_{C}$ = ' + str(Rc_anno) + r' $\pm$ ' + str(delRc_anno) + r' $\Omega$.$\mu$m',
                xy=(annotate_x, annotate_y1), xycoords='axes fraction')
...
Rtot = TLM_data[:,1] #Channel lengths (ohm)
...
results = model.fit()
Rc = results.params[0] / 2  # Contact resistance (kOhm.um)
print(Rc)
del_Rc = results.bse[0] / 2  # Error in Contact resistance (kOhm.um)
print(del_Rc)
...
# fig.savefig("peideye_ITO_analysis.svg", transparent=True, bbox_inches='tight', pad_inches=0.1, dpi = 300)
# fig.savefig("peideye_ITO_analysis.png", bbox_inches='tight', pad_inches=0.1)

# fig.savefig("nagoya_1e19.svg", transparent=True, bbox_inches='tight', pad_inches=0.1, dpi = 300)
# fig.savefig("nagoya_1e19.png", bbox_inches='tight', pad_inches=0.1)

fig.savefig("nagoya_1e20.svg", transparent=True, bbox_inches='tight', pad_inches=0.1, dpi = 300)
fig.savefig("nagoya_1e20.png", bbox_inches='tight', pad_inches=0.1)