...
custom_colors[2] = '#808080'
custom_colors[3] = '#1ac938'
...
user_folder = "/Users/tangalvin9/Desktop/Windows10_Shared/TMD Device Data/"
...
dir_path = "/20200131-IA3"; Vg_idvd_plt = 20
...
vd_label = r'${V}_{DS}$ [V]'
vg_label = r'${V}_{GS}$ [V]'
vov_label = r'${V}_{ov}$ [V]'
n2d_label = r'${n}$ [$10^{12}$ $cm^{-2}$]'
id_label = r'${I}_D$ [$\mu A$/$\mu$$m$]'
rc_label = r'${R}_{C}$ [k$\Omega.\mu$$m$]'
mu_eff_label = r'${\mu}_{eff}$ [$cm^2$$V^{-1}$$s^{-1}$]'
rsh_label = r'${R}_{sh}$ [k$\Omega.\boxdot$]'
rtot_label = r'${R}_{tot}$ [k$\Omega.\mu$$m$]'
lch_label = r'${L}_{ch}$ [$\mu$$m$]'
annotate_x = 0.05
annotate_y1 = 0.85
annotate_y2 = 0.75
annotate_y22 = 0.725
...
class TLM:
    ...
    def __init__(self, inp_data, inp_count):
        ...
        self.Vds_idvg = self.idvg[0,0].Vds  # Vds forId-Vg sweep
        self.Vds_idvg_hi = self.idvg[0,-1].Vds  # Vds forId-Vg sweep
        ...
        self.Id_idvg_hiVds = np.zeros([num_vg, self.count])
    ...
    def tlm_calc(self):
        ...
        for i in np.arange(self.count):
            self.Vov[:, i] = self.idvg[i,0].Vov
            # self.Vov[:, i] = self.idvg[i].Vov
            self.Id_idvg_forward[:, i] = self.idvg[i,0].Id_forward
            # self.Id_idvg_forward[:, i] = self.idvg[i].Id_forward
            self.Id_idvg[:, i] = self.idvg[i,0].Id
            self.Id_idvg_hiVds[:, i] = self.idvg[i,-1].Id
        ...
        for i in np.arange(self.count):
            self.Id_adjusted[:, i] = self.idvg[i,0].Id_forward[index[:, i]]
    ...
    def plot_IdVg(self, ax, fig, lg='lin', vd_step='low'):
        ...
        if vd_step == 'low':
            ...
            ax.annotate(r'$V_{DS}$ = ' + str(self.Vds_idvg_hi) + ' V', xy=(.65, .65), xycoords='axes fraction')
            ax.plot(self.Vg_idvg, self.Id_idvg_hiVds * 1e6)  # Plotting Id (uA/um) vs Vgs (V)
    ...
    def plot_IdVg_multVds(self, ax, fig, lg='lin',channel=0):
        ...
        legends = [r'$V_{DS}$ = ' + str(self.Vds_idvg) + ' V', r'$V_{DS}$ = ' + str(self.Vds_idvg_hi) + ' V']
        ax.legend(legends, loc='lower right',bbox_to_anchor=[0.95,0.0])
