...
def xls_read(filename_g):
    ...
    idvg_data = np.asarray(
        pd.read_excel(xlrd.open_workbook(filename_g,
                                         logfile=open(os.devnull, 'w')),
                      engine='xlrd'))  # Reading Id-Vg data
    return idvg_data
...
def plot_properties(mpl):
    ...
    mpl.rcParams[
        'figure.subplot.wspace'] = 0.3  # Horizontal spacing in subplots
    ...
    mpl.rcParams[
        'figure.constrained_layout.h_pad'] = 0.04167  # Padding around axes objects
...
def read_params(target_dir,
    ...
    channel_params = xlrd.open_workbook(channel_params,
                                        logfile=open(os.devnull, 'w'))
    channel_params = np.asarray(pd.read_excel(
        channel_params, engine='xlrd'))  #Reading Id-Vg data

    #Reading device parameters
    device_params = Path(
        device_params_file
    )  #Device parameters file - type of sweep, column indices, etc.
    device_params = xlrd.open_workbook(device_params,
                                       logfile=open(os.devnull, 'w'))
    device_params = np.asarray(pd.read_excel(
        device_params, engine='xlrd'))  #Reading Id-Vg data
    ...
    isfolder_organized = device_params[:,
                                       4]  #Whether the device data is organized into folders
...
def set_x_ticks(ax, num_ticks=5, round_factor=None):
    ...
    tick_frequency = round(
        (plot_range / num_ticks) / round_factor) * round_factor
...
def plot_by_Lch(fig, ax, device_count, tlm_set, channel_L, channel_label,
                num_channels):
    ...
    for i in np.arange(device_count):
        for k in np.arange(tlm_set[i].count):
            ...
            if tlm_set[i].idvg[k, 0].gateLeak == 0:
                tlm_set[i].plot_IdVg_multVds(ax[ax_index],
                                             fig,
                                             'log',
                                             channel=k,
                                             anno='no')
...
def plot_cdf(fig, ax, device_count, tlm_set, get_function, n2D=1e13):
    ...
    goodTLM_count = 0
    for i in np.arange(device_count):
        if tlm_set[i].gateLeak == 0 and tlm_set[
                i].goodTLM and tlm_set[i].R_squared >= 0.8:
            X, del_X, R_squared, goodX, xaxis_label = get_function(tlm_set[i],
                                                                   n2D=n2D)
...
def plot_all_Rtot(fig, ax, device_count, tlm_set, n2D=1e13):
    ...
    n2D_anno = n2D / 1e12
    if n2D_anno < 10:
        ax.annotate(r'at $n_{2D}$ = ' + str(np.round(n2D_anno, 1)) +
                    r' $\times$ $10^{12} cm^{-2}$',
                    xy=(annotate_x, annotate_y22),
                    xycoords='axes fraction',
                    fontsize=8.5)
    else:
        ax.annotate(r'at $n_{2D}$ = ' + str(np.round(n2D_anno / 10, 1)) +
                    r' $\times$ $10^{13} cm^{-2}$',
                    xy=(annotate_x, annotate_y22),
                    xycoords='axes fraction',
                    fontsize=8.5)
...
def plot_gate_leakage(fig_gate, ax_gate, device_count, tlm_set, channel_L,
                      channel_length):
...
class IdVg:
    ...
    def __init__(self, inp, params,
                 step_number):  # Argument is of type InputData
        """
        ...
        if isbipolar_idvg:
            self.num_vg_forward = int(inp.id_vg.shape[0] /
                                      2)  # Number of Vg points in
        else:
            self.num_vg_forward = int(
                inp.id_vg.shape[0])  # Number of Vg points in

        self.Id = inp.id_vg[0:self.num_vg, col_index[0] + step_number *
                            num_var_idvg] / W  # Reading drain current (A/um)
        self.Vg = inp.id_vg[0:self.num_vg, col_index[1] + step_number *
                            num_var_idvg]  # Reading gate voltage (V)
        self.Ig = inp.id_vg[0:self.num_vg, col_index[6] + step_number *
                            num_var_idvg]  # Reading gate current (A/um)

        if np.max([
                np.max(np.abs(self.Id)),
                np.max(np.abs(self.Ig)),
                np.max(np.abs(self.Vg))
        ]) > compliance_threshold:
            self.compliance = 1

        self.Id_forward = inp.id_vg[0:self.num_vg_forward, col_index[0] +
                                    step_number * num_var_idvg] / W  #(A/um)
        # Parsing only forward sweep drain current (A/um)
        self.Vg_forward = inp.id_vg[0:self.num_vg_forward, col_index[1] +
                                    step_number * num_var_idvg]  #(V)

        if params.interp:
            Vg_min = np.round(np.min(self.Vg), 0)
            Vg_max = np.round(np.max(self.Vg), 0)
            Vg_interp = np.arange(Vg_min,
                                  Vg_max + 1)  # Interpolate with dV step = 1
            Id_interp = np.interp(Vg_interp, self.Vg_forward, self.Id_forward)
            self.Id_forward = Id_interp  #(A/um)
            self.Vg_forward = Vg_interp  #(V)
            self.num_vg_forward = int(
                self.Id_forward.shape[0])  # Number of Vg points in

        self.Id_max_idvg = np.amax(np.absolute(self.Id)) * 1e6  #uA/um
        # Parsing only forward sweep gate voltage (V)

        if params.Vds_param == None:
            self.Vds = np.round(inp.id_vg[0, col_index[2] +
                                          step_number * num_var_idvg],
                                1)  # Reading Vds from Id-Vg data (V)
        else:
            self.Vds = params.Vds_param

        self.I_off = np.maximum(self.Id_forward[0],
                                I_noise / W)  # Off current (A/um)
    ...
    def idvg_calc(self, params):
        ...
        Cox = params.Cox
        W = params.W
        self.on_off_ratio = self.Id_forward[
            -1] / self.I_off  # On-off ratio (no units)
        # Off current is minimum(Id, noise floor=1e-12A)
        self.SS = np.diff(self.Vg_forward) / np.diff(
            np.log10(
                np.absolute(self.Id_forward.astype(float)) + np.spacing(1)) +
            np.spacing(1)) * 1e3  # Subthreshold slope (mV/dec)
        # Calculating field-effect mobility
        self.mu_FE = np.zeros((self.Vg_forward.shape[0] - 1))
        self.gm = np.diff(smooth(self.Id_forward, 5)) / np.diff(
            self.Vg_forward)  # Transconductance (S/um)
        self.dgm = np.diff(smooth(self.gm, 5)) / np.diff(
            self.Vg_forward[:-1])  # Transconductance derivative (S/V/um)
        self.dgm = smooth(self.dgm,
                          5)  # Smoothed Transconductance derivative (S/V/um)
        self.gm = smooth(self.gm)  # Smoothed Transconductance (S/um)
        self.mu_FE = self.L * self.gm / (
            Cox * self.Vds * 1e-9)  # Possible under-estimation? (cm2/V/s)

        # Calculating Constant Current Vt ##
        if np.amin(np.absolute(self.Id_forward)) < Id_thresh * W / self.L:
            self.Vt_const = np.amax(
                self.Vg_forward[self.Id_forward < Id_thresh * W /
                                self.L])  # Constant current Vt (V)

        gmmax_index = np.where(
            self.gm == np.amax(self.gm))[0]  #Index of maximum gm
        dgmmax_index = np.where(self.dgm[:-9] == np.amax(
            self.dgm[:-9]))  # Index of maximum dgm
        dgmmax_index, _ = find_peaks(
            self.dgm, height=0)  # Finds the first local max in dgm

        if self.Id_max_idvg < 1e-1 and self.on_off_ratio < 1e2:  #Setting gate leakage threshold at 1e-7
            self.goodIdVg = 0

        if (np.amax(np.absolute(self.Ig)) >
                1e-7):  #Setting gate leakage threshold at 1e-7
            self.gateLeak = 1

        if (dgmmax_index.shape[0] > 0):
            # Calculating Max Transconductance Derivative Vt ##
            dgmmax_index = dgmmax_index[0]
            self.Vt_dgm = self.Vg_forward[
                dgmmax_index]  # Threshold voltage from max derivative of gm (V)
            Vt = self.Vt_dgm
        if (gmmax_index.shape[0] > 0):
            # elif (gmmax_index.shape[0] > 0):
            # Calculating Linear Interpolation Vt ##
            gmmax_index = gmmax_index[0]
            Vg_intercept = self.Vg_forward[
                gmmax_index] - self.Id_forward[gmmax_index] / np.amax(self.gm)
            Vt_lin_temp = Vg_intercept - self.Vds / 2  # Linear extrapolated Vt (V)
            self.Vt_lin = find_nearest(
                self.Vg_forward, Vt_lin_temp
            )  # Rounding the Vt_lin to the nearest element in Vg array
            Vt = self.Vt_lin
    ...
    def plot_IdVg(self, ax, fig, lg='lin', anno='no'):
        ...
        if anno == 'yes':
            ax.annotate(r'$V_{DS}$ = ' + str(self.Vds) + ' V',
                        xy=(.45, annotate_y1),
...
class IdVd:
    ...
    def __init__(self, inp, params):  # Argument is of type InputData
        ...
        if isbipolar_idvd:  # If bipolar sweep, just take forward sweep, else dual sweep
            ...
            num_vd = int(inp.id_vd.shape[0])
        self.Vd = np.round(inp.id_vd[0:num_vd, col_index[5]],
                           2)  # Drain voltage (V)
        self.Id = inp.id_vd[
            0:num_vd, col_index[3]:num_vg_idvd *
            num_var_idvd:num_var_idvd] / W  # Drain current (A/um)
        self.Vg = inp.id_vd[0, col_index[4]:num_vg_idvd *
                            num_var_idvd:num_var_idvd]
        self.channel_length = inp.channel_length  # Channel length string
        self.L = inp.channel_L  # Channel length (um)
        ...
        if self.L < 1:
            ...
            self.L_label = str(self.L) + r' $\mu$m'
        self.Id_max = np.amax(
            np.abs(inp.id_vd[:, col_index[3] + num_var_idvd *
                             (num_vg_idvd - 1)] / W *
                   1e6))  # Maximum drain current (uA/um)
    ...
    def plot_IdVd(self, ax, fig, params, Vgs=None):
        ...
        if Vgs is None:  # When plot_IdVd is called directly, not through TLM
            ...
            ax.annotate(r'$L_{ch}$ = ' + self.L_label,
                        xy=(annotate_x, annotate_y1),
                        xycoords='axes fraction')
            ax.plot(self.Vd, self.Id *
                    1e6)  # Plotting Id (uA/um) vs Vds (V) for alL Vgs
            ax.locator_params(nbins=6, axis='y')
...
class TLM:
    ...
    def __init__(self, inp_data, inp_count, params):
        ...
        self.idvd = np.empty(shape=(self.count, ),
                             dtype=object)  # Id-Vd object
        self.L = np.empty(shape=(self.count, ),
                          dtype=float)  # Channel lengths in the TLM
        self.L_label = np.empty(
            shape=(self.count, ),
            dtype=object)  # Label strings to use for plotting
        self.gateLeak = 0  #Flag to indicate gate leakage
        self.compliance = 0  #Flag to indicate whether device hit compliance
        self.position = str(self.row) + '-' + str(
            self.column)  #TLM position string
        ...
        for i in np.arange(self.count):
            ...
            for j in np.arange(num_vd_idvg):
                self.idvg[goodIdVg_count,
                          j] = IdVg(self.data[i], self.params,
                                    step_number=j)  # Id-Vg object
                self.idvg[goodIdVg_count, j].idvg_calc(
                    self.params)  # Perform calculations on Id-Vg
            ...
            if goodIdVg_flag == 0:
                ...
                if self.data[goodIdVg_count].id_vd is None:
                    ...
                    self.idvd[goodIdVg_count] = IdVd(
                        self.data[i], self.params)  # Id-Vd object
                ...
                if self.L[goodIdVg_count] < 1:
                    self.L_label[goodIdVg_count] = str(
                        int(self.L[goodIdVg_count] * 1000)) + r' nm'
                else:
                    self.L_label[goodIdVg_count] = str(
                        self.L[goodIdVg_count]) + r' $\mu$m'
        ...
        )  # Number of Vg points in forward sweep of Id-Vg data
        # num_vg_forward = int(self.idvg[0].num_vg_forward)  # Number of Vg points in forward sweep of Id-Vg data
        num_vg = int(self.idvg[0,
                               0].num_vg)  # Number of Vg points in Id-Vg data
        # num_vg = int(self.idvg[0].num_vg)  # Number of Vg points in Id-Vg data
        self.Vds_idvg = [self.idvg[0, 0].Vds,
                         self.idvg[0, -1].Vds]  # Vds forId-Vg sweep
        self.Vov = np.zeros([num_vg_forward,
                             self.count])  # Overdrive voltage (V)
        self.Vg_idvg = self.idvg[0, 0].Vg  # Vg used in Id-Vg (V)
        self.Vg_idvg_forward = self.idvg[0,
                                         0].Vg_forward  # Vg used in Id-Vg (V)
        ...
        self.TLM_fit = [
        ]  # Pandas dataframe to store regression data @ highest Vov
    ...
    def tlm_calc(self, vd_step='low'):
        ...
        Vov_hi = np.amin(
            self.Vov.max(0))  # Higher and lower limits of Vov (or Vgs-Vt)
        ...
        self.mu_FE_adjusted = np.zeros([self.Vov.shape[0] - 1,
                                        self.count])  #(cm2/V/s)
        self.gm_adjusted = np.zeros([self.Vov.shape[0] - 1,
                                     self.count])  #(S/um)

        if self.Vov.shape[
                0] > 0 and self.count > 1 and self.gateLeak == 0 and self.compliance == 0:
            self.goodTLM = 1
        ...
        if self.goodTLM:
            for i in np.arange(self.count):
                self.Id_adjusted[:, i] = np.interp(
                    self.Vov, self.idvg[i, j].Vov,
                    self.idvg[i, j].Id_forward)  #(A/um)
                self.mu_FE_adjusted[:, i] = np.interp(
                    self.Vov[:-1], self.idvg[i, j].Vov[:-1],
                    self.idvg[i, j].mu_FE)  #(cm2/V/s)
                self.gm_adjusted[:, i] = np.interp(self.Vov[:-1],
                                                   self.idvg[i, j].Vov[:-1],
                                                   self.idvg[i, j].gm)  #(S/um)

            # Calculating Rc, del_Rc, Rsh, del_Rsh
            self.Rtot = self.Vds_idvg[
                j] / self.Id_adjusted  # Total resistance (Ohm.um)
            ...
            for i in np.arange(self.Vov.shape[0]):
                X = sm.add_constant(self.L.conj().transpose()
                                    )  # Adding constant term to the model
                model = sm.OLS(self.Rtot[i, :].conj().transpose(), X)
                results = model.fit()
                self.Rc[
                    i] = results.params[0] / 2  # Contact resistance (Ohm.um)
                self.del_Rc[i] = results.bse[
                    0] / 2  # Error in Contact resistance (Ohm.um)
                if ~(np.isfinite(self.del_Rc[i])):
                    self.del_Rc[i] = 0
                self.Rsh[i] = results.params[1]  # Sheet resistance (Ohm/sq)
                self.del_Rsh[i] = results.bse[
                    1]  # Error in Sheet resistance (Ohm/sq)
                if ~(np.isfinite(self.del_Rsh[i])):
                    self.del_Rsh[i] = 0
            self.R_squared = results.rsquared
            d = {
                rtot_label: self.Rtot[i, :].conj().transpose() / 1e3,
                lch_label: self.L.conj().transpose()
            }  # Creating dictionary of Rtot vs L
            self.TLM_fit = pd.DataFrame(
                data=d)  # Converting dictionary to Pandas dataframe
            self.n2D = Cox * 1e-9 * (self.Vov + np.spacing(1)
                                     ) / q  # 2D carrier concentration (cm-2)
            # np.spacing(1) is used to avoid divide-by-zero error in next step
            self.mu_TLM = 1 / ((self.Rsh * self.n2D) * q + np.spacing(1)
                               )  # Mobility extracted from TLM (cm^2/V/s)
            self.del_mu_TLM = self.del_Rsh / (
                self.Rsh + np.spacing(1)
            ) * self.mu_TLM  # Error in mobility extracted from TLM (cm^2/V/s)
            if (self.Rc[-1] > 0) and (self.Rc[-1] >
                                      2 * np.abs(self.del_Rc[-1])):
                self.goodRc = 1
            if (self.mu_TLM[-1] > 0) and (self.mu_TLM[-1] >
                                          2 * np.abs(self.del_mu_TLM[-1])):
                self.goodMobility = 1
    ...
    def plot_TLM_fit(self, ax, fig):
        ...
        ax.set_xlim(left=0, right=round(self.L[-1] * 1.1 / 0.2) *
                    0.2)  # Sets lower x-limit to 0
        ...
        ax.annotate(r'$R_{C}$ = ' + str(Rc_anno) + r' $\pm$ ' +
                    str(delRc_anno) + r' k$\Omega$.$\mu$m',
                    xy=(annotate_x, annotate_y1),
                    xycoords='axes fraction')
    ...
    def plot_mu_FE(self, ax, fig, channel=0, flag=None, legend='no'):
        ...
        if flag is None:
            ...
            if channel == 0:
                ...
                for i in np.arange(self.count):
                    ax.plot(Vov_temp[Vov_temp >= 0],
                            self.mu_FE_adjusted[Vov_temp >= 0, i])
                legends = [i for i in self.L_label]
            elif channel == -1:
                """plot mu_FE for longest channels"""
                ax.plot(Vov_temp[Vov_temp >= 0],
                        self.mu_FE_adjusted[Vov_temp >= 0, -1])
                legends = [self.L_label[-1]]
            elif channel > 0:
                """plot mu_FE for the specified channel"""
                ax.plot(Vov_temp[Vov_temp >= 0],
                        self.mu_FE_adjusted[Vov_temp >= 0, channel - 1])
        ...
        # Plotting mu_FE (cm2/V/s) vs n2D(V)
            ...
            if channel == 0:
                """plot mu_FE for all channels"""
                for i in np.arange(self.count):
                    ax.plot(n2D_temp[n2D_temp >= n2D_LL] / 1e12,
                            self.mu_FE_adjusted[n2D_temp >= n2D_LL, i])
                legends = [i for i in self.L_label]
            elif channel == -1:
                """plot mu_FE for longest channels"""
                ax.plot(n2D_temp[n2D_temp >= n2D_LL] / 1e12,
                        self.mu_FE_adjusted[n2D_temp >= n2D_LL, -1])
                legends = [self.L_label[-1]]
            elif channel > 0:
                """plot mu_FE for the specified channel"""
                ax.plot(n2D_temp[n2D_temp >= n2D_LL] / 1e12,
                        self.mu_FE_adjusted[n2D_temp >= n2D_LL, channel - 1])
                legends = [self.L_label[channel - 1]]
        if legend == 'yes':
            ax.legend(legends, loc='lower right', bbox_to_anchor=[0.95, 0.0])
        return legends
    ...
    def plot_mu_TLM(self, ax, fig, flag=None):
        ...
        if flag is None:
            ...
            ax.errorbar(self.n2D[self.n2D >= n2D_LL] / 1e12,
                        self.mu_TLM[self.n2D >= n2D_LL],
    ...
    def plot_gm_aligned(self, ax, fig, channel=0, flag=None, legend='no'):
        ...
        if flag is None:
            ...
            if channel == 0:
                ...
                for i in np.arange(self.count):
                    ax.plot(Vov_temp[Vov_temp >= 0],
                            self.gm_adjusted[Vov_temp >= 0, i])
                legends = [i for i in self.L_label]
            elif channel == -1:
                """plot gm for longest channels"""
                ax.plot(Vov_temp[Vov_temp >= 0],
                        self.gm_adjusted[Vov_temp >= 0, -1])
                legends = [self.L_label[-1]]
            elif channel > 0:
                """plot gm for the specified channel"""
                ax.plot(Vov_temp[Vov_temp >= 0],
                        self.gm_adjusted[Vov_temp >= 0, channel - 1])
                legends = [self.L_label[channel - 1]]
        # Plotting gm (cm2/V/s) vs n2D(V)
            ...
            if channel == 0:
                ...
                """plot gm for longest channels"""
                ax.plot(n2D_temp[n2D_temp >= n2D_LL] / 1e12,
                        self.gm_adjusted[n2D_temp >= n2D_LL, -1])
                legends = [self.L_label[-1]]
            elif channel > 0:
                """plot gm for the specified channel"""
                ax.plot(n2D_temp[n2D_temp >= n2D_LL] / 1e12,
                        self.gm_adjusted[n2D_temp >= n2D_LL, channel - 1])
                legends = [self.L_label[channel - 1]]
        if legend == 'yes':
            ax.legend(legends, loc='lower right', bbox_to_anchor=[0.95, 0.0])
        return legends
    ...
    def get_Rc(self, n2D=1e13):
        ...
        goodRc = 0
        if Rc_interp > 0 and Rc_interp < 1e4 and np.abs(Rc_interp) > np.abs(
                del_Rc_interp):
            goodRc = 1
    ...
    def get_mu_TLM(self, n2D=1e13):
        ...
        goodmu_TLM = 0
        if mu_TLM_interp > 0 and np.abs(mu_TLM_interp) > np.abs(
                del_mu_TLM_interp):
            goodmu_TLM = 1
    ...
    def plot_Rtot(self, ax, fig, n2D=1e13, anno='yes'):
        ...
        if anno == 'yes':
            ...
            if n2D_anno < 10:
                ...
                ax.annotate(r'at $n_{2D}$ = ' +
                            str(np.round(n2D_anno / 10, 1)) +
                            r' $\times$ $10^{13} cm^{-2}$',
                            xy=(annotate_x, annotate_y22),
                            xycoords='axes fraction',
                            fontsize=8.5)
        for j in np.arange(self.count):
            ...
            ax.plot(self.L[j], np.abs(Rtot_interp / 1e3), 'o',
                    color='black')  #Plotting Rtot(kohm.um) vs Lch (um)
        pass
    ...
    def plot_IdVg(self, ax, fig, lg='lin', vd_step='low'):
        ...
        if vd_step == 'low':
            ...
            ax.annotate(r'$V_{DS}$ = ' + str(self.Vds_idvg[j]) + ' V',
                        xy=(annotate_x, annotate_y1),
                        xycoords='axes fraction')
            ax.plot(self.Vg_idvg, self.Id_idvg[:, :, j] *
                    1e6)  # Plotting Id (uA/um) vs Vgs (V)
        elif vd_step == 'hi':
            j = -1
            # ax.annotate(r'$V_{DS}$ = ' + str(self.Vds_idvg[j]) + ' V', xy=(.65, .65), xycoords='axes fraction')
            ax.annotate(r'$V_{DS}$ = ' + str(self.Vds_idvg[j]) + ' V',
    ...
    def plot_IdVg_multVds(self, ax, fig, lg='lin', channel=0, anno='no'):
        ...
        if anno == 'yes':
            ax.annotate(r'$L_{CH}$ = ' + self.idvg[channel, 0].L_label,
                        xy=(annotate_x, annotate_y1),
                        xycoords='axes fraction')
        num_vd_idvg = self.params.num_vd_idvg
        self.idvg[channel, 0].plot_IdVg(
            ax, fig, lg)  # Plotting Id-Vg for all channel lengths in log scale
        if num_vd_idvg > 1:
            self.idvg[channel, -1].plot_IdVg(
                ax, fig,
                lg)  # Plotting Id-Vg for all channel lengths in log scale
            legends = [
                r'$V_{DS}$ = ' + str(self.Vds_idvg[0]) + ' V',
                r'$V_{DS}$ = ' + str(self.Vds_idvg[-1]) + ' V'
            ]
        else:
    ...
    def plot_IdVg_Vov(self, ax, fig, lg='lin'):
        ...
        ax.annotate(r'$V_{DS}$ = ' + str(self.Vds_idvg) + ' V',
                    xy=(.15, annotate_y1),
                    xycoords='axes fraction')
        ...
        if lg == 'log':
            ax.set_yscale('log')
        ax.plot(self.Vov,
                self.Id_adjusted * 1e6)  # Plotting Id (uA/um) vs Vgs (V)
        legends = [i for i in self.L_label]
        ax.legend(legends, loc='lower right', bbox_to_anchor=[0.95, 0.0])
    ...
    def summary_plot(self, ax, fig, device_name):
        ...
        6) mu_TLM plot
        """
        self.plot_IdVg(ax[0], fig, lg='log', vd_step='low'
                       )  # Plotting Id-Vg for all channel lengths in log scale
        self.plot_IdVg(ax[1], fig, lg='log', vd_step='hi'
                       )  # Plotting Id-Vg for all channel lengths in log scale
        self.plot_IdVg_multVds(ax[2], fig, 'log',
                               channel=0)  # Plotting Id-Vg for 100nm in log
        if self.goodTLM:
