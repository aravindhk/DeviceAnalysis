...
def xls_read(filename_g):
    ...
    idvg_data = np.asarray(
        pd.read_excel(
            xlrd.open_workbook(filename_g, logfile=open(os.devnull, "w")), engine="xlrd"
        )
    )  # Reading Id-Vg data
    return idvg_data
...
def plot_properties(mpl):
    ...
    ] = 0.04167  # Padding around axes objects
...
def read_params(
    ...
    device_params = np.asarray(
        pd.read_excel(device_params, engine="xlrd")
    )  # Reading Id-Vg data
...
def set_x_ticks(ax, num_ticks=5, round_factor=None):
    ...
    ax.xaxis.set_ticks(
        np.arange(
            round(start / tick_frequency) * tick_frequency,
            (round(end / tick_frequency) + 1) * tick_frequency,
            tick_frequency,
        )
    )
...
def plot_heatmaps(
    ...
    if open_device is not None:
        ...
        fig3.savefig(
            "open.svg", transparent=True, bbox_inches="tight", dpi=300, pad_inches=0.1
        )
    ...
    if short_device is not None:
        ...
        fig4.savefig(
            "short.svg", transparent=True, bbox_inches="tight", dpi=300, pad_inches=0.1
        )
    ...
    for i in np.arange(device_count):
        for k in np.arange(tlm_set[i].count):
            ...
            if tlm_set[i].idvg[k, 0].gateLeak == 0:
                tlm_set[i].plot_IdVg_multVds(
                    ax[ax_index], fig, "log", channel=k, anno="no"
                )
...
def plot_cdf(fig, ax, device_count, tlm_set, get_function, n2D=1e13):
    ...
    for i in np.arange(device_count):
        if (
            tlm_set[i].gateLeak == 0
            and tlm_set[i].goodTLM
            and tlm_set[i].R_squared >= 0.8
        ):
    ...
    rc_dataset = pd.DataFrame(
        {
            "x": X_array[:goodTLM_count],
            "del_x": del_X_array[:goodTLM_count],
            "r_squared": R_squared_array[:goodTLM_count],
...
def plot_gate_leakage(
    fig_gate, ax_gate, device_count, tlm_set, channel_L, channel_length
):
...
class IdVg:
    ...
    def __init__(self, inp, params, step_number):  # Argument is of type InputData
        ...
        if params.interp:
            ...
            self.num_vg_forward = int(
                self.Id_forward.shape[0]
            )  # Number of Vg points in
    ...
    def idvg_calc(self, params):
        ...
        self.SS = (
            np.diff(self.Vg_forward)
            / np.diff(
                np.log10(np.absolute(self.Id_forward.astype(float)) + np.spacing(1))
                + np.spacing(1)
            )
            * 1e3
        ...
        self.dgm = np.diff(smooth(self.gm, 5)) / np.diff(
            self.Vg_forward[:-1]
        )  # Transconductance derivative (S/V/um)
        ...
        self.mu_FE = (
            self.L * self.gm / (Cox * self.Vds * 1e-9)
        )  # Possible under-estimation? (cm2/V/s)

        # Calculating Constant Current Vt ##
        if np.amin(np.absolute(self.Id_forward)) < Id_thresh * W / self.L:
            self.Vt_const = np.amax(
                self.Vg_forward[self.Id_forward < Id_thresh * W / self.L]
            )  # Constant current Vt (V)

        gmmax_index = np.where(self.gm == np.amax(self.gm))[0]  # Index of maximum gm
        dgmmax_index = np.where(
            self.dgm[:-9] == np.amax(self.dgm[:-9])
        )  # Index of maximum dgm
        dgmmax_index, _ = find_peaks(
            self.dgm, height=0
        )  # Finds the first local max in dgm

        if (
            self.Id_max_idvg < 1e-1 and self.on_off_ratio < 1e2
        ):  # Setting gate leakage threshold at 1e-7
            self.goodIdVg = 0

        if (
            np.amax(np.absolute(self.Ig)) > 1e-7
        ):  # Setting gate leakage threshold at 1e-7
            self.gateLeak = 1
...
class IdVd:
    ...
    def plot_IdVd(self, ax, fig, params, Vgs=None):
        ...
        if Vgs is None:  # When plot_IdVd is called directly, not through TLM
            ...
            ax.annotate(
                r"$L_{ch}$ = " + self.L_label,
                xy=(annotate_x, annotate_y1),
                xycoords="axes fraction",
            )
            ax.plot(
                self.Vd, self.Id * 1e6
            )  # Plotting Id (uA/um) vs Vds (V) for alL Vgs
            ax.locator_params(nbins=6, axis="y")
...
class TLM:
    ...
    def __init__(self, inp_data, inp_count, params):
        ...
        for i in np.arange(self.count):
            ...
            for j in np.arange(num_vd_idvg):
                self.idvg[goodIdVg_count, j] = IdVg(
                    self.data[i], self.params, step_number=j
                )  # Id-Vg object
                self.idvg[goodIdVg_count, j].idvg_calc(
                    self.params
                )  # Perform calculations on Id-Vg
            ...
            if goodIdVg_flag == 0:
                ...
                if self.L[goodIdVg_count] < 1:
                    self.L_label[goodIdVg_count] = (
                        str(int(self.L[goodIdVg_count] * 1000)) + r" nm"
                    )
                else:
                    self.L_label[goodIdVg_count] = (
        ...
        if self.goodTLM:
            for i in np.arange(self.count):
                ...
                self.mu_FE_adjusted[:, i] = np.interp(
                    self.Vov[:-1], self.idvg[i, j].Vov[:-1], self.idvg[i, j].mu_FE
                )  # (cm2/V/s)
                self.gm_adjusted[:, i] = np.interp(
                    self.Vov[:-1], self.idvg[i, j].Vov[:-1], self.idvg[i, j].gm
                )  # (S/um)
            ...
            for i in np.arange(self.Vov.shape[0]):
                ...
                self.del_Rc[i] = (
                    results.bse[0] / 2
                )  # Error in Contact resistance (Ohm.um)
            ...
            self.n2D = (
                Cox * 1e-9 * (self.Vov + np.spacing(1)) / q
            )  # 2D carrier concentration (cm-2)
            # np.spacing(1) is used to avoid divide-by-zero error in next step
            self.mu_TLM = 1 / (
                (self.Rsh * self.n2D) * q + np.spacing(1)
            )  # Mobility extracted from TLM (cm^2/V/s)
            self.del_mu_TLM = (
                self.del_Rsh / (self.Rsh + np.spacing(1)) * self.mu_TLM
            )  # Error in mobility extracted from TLM (cm^2/V/s)
            if (self.Rc[-1] > 0) and (self.Rc[-1] > 2 * np.abs(self.del_Rc[-1])):
                self.goodRc = 1
            if (self.mu_TLM[-1] > 0) and (
                self.mu_TLM[-1] > 2 * np.abs(self.del_mu_TLM[-1])
            ):
                self.goodMobility = 1
    ...
    def plot_TLM_fit(self, ax, fig):
        ...
        ax.set_xlim(
            left=0, right=round(self.L[-1] * 1.1 / 0.2) * 0.2
        )  # Sets lower x-limit to 0
        sns.regplot(
            x=lch_label, y=rtot_label, data=self.TLM_fit, ci=95, truncate=False, ax=ax
        )
        ...
        ax.annotate(
            r"R-squared = " + str(np.round(self.R_squared, 3)),
            xy=(0.6, 0.1),
            xycoords="axes fraction",
            fontsize=8.5,
        )
    ...
    def plot_mu_TLM(self, ax, fig, flag=None):
        ...
        top_limit = np.maximum(2 * mu_TLM_cut[0], mu_TLM_cut[0] + del_mu_TLM_cut[0])
    ...
    def get_Rc(self, n2D=1e13):
        ...
        if (
            Rc_interp > 0
            and Rc_interp < 1e4
            and np.abs(Rc_interp) > np.abs(del_Rc_interp)
        ):
            goodRc = 1
    ...
    def plot_Rtot(self, ax, fig, n2D=1e13, anno="yes"):
        ...
        for j in np.arange(self.count):
            ...
            )  # Plotting Rtot(kohm.um) vs Lch (um)
        pass
            ...
            try:
                ax.set_ylim(
                    bottom=(
                        np.minimum(
                            np.amin(
                                self.Rc[self.n2D >= n2D_LL]
                                - self.del_Rc[self.n2D >= n2D_LL]
                            )
                            / 1e3,
                            0,
                        )
        ...
        if flag is None:
            ...
            ax.set_ylim(
                bottom=(
                    np.minimum(
                        np.amin(
                            self.Rsh[self.n2D >= n2D_LL]
                            - self.del_Rsh[self.n2D >= n2D_LL]
                        )
                        / 1e3,
                        0,
                    )
            ...
            ax.annotate(
                r"at $n_{2D}$ = "
                + str(np.round(self.n2D[-1] / 1e12, 1))
                + r" $\times$ $10^{12} cm^{-2}$",
                xy=(0.25, annotate_y22),
                xycoords="axes fraction",
                fontsize=8.5,
            )
    ...
    def plot_IdVg(self, ax, fig, lg="lin", vd_step="low"):
        ...
        if vd_step == "low":
            ...
            ax.annotate(
                r"$V_{DS}$ = " + str(self.Vds_idvg[j]) + " V",
                xy=(annotate_x, annotate_y1),
                xycoords="axes fraction",
            )
            ax.plot(
                self.Vg_idvg, self.Id_idvg[:, :, j] * 1e6
            )  # Plotting Id (uA/um) vs Vgs (V)
        elif vd_step == "hi":
            j = -1
            # ax.annotate(r'$V_{DS}$ = ' + str(self.Vds_idvg[j]) + ' V', xy=(.65, .65), xycoords='axes fraction')
            ax.annotate(
    ...
    def plot_IdVg_multVds(self, ax, fig, lg="lin", channel=0, anno="no"):
        ...
        if anno == "yes":
            ax.annotate(
                r"$L_{CH}$ = " + self.idvg[channel, 0].L_label,
                xy=(annotate_x, annotate_y1),
                xycoords="axes fraction",
            )
        num_vd_idvg = self.params.num_vd_idvg
    ...
    def plot_IdVd(self, ax, fig, Vgs):
        ...
        ax.annotate(
            r"$I_{D,max}$ = " + str(idmax_anno) + r" $\mu A / \mu m$",
            xy=(0.03, annotate_y22),
            xycoords="axes fraction",
        )
        ax.legend(legends, loc="lower right", bbox_to_anchor=[0.95, 0.0])
        ax.set(xlabel=vd_label, ylabel=id_label)
    ...
    def summary_plot(self, ax, fig, device_name):
        ...
        """
        self.plot_IdVg(
            ax[0], fig, lg="log", vd_step="low"
        )  # Plotting Id-Vg for all channel lengths in log scale
        self.plot_IdVg(
            ax[1], fig, lg="log", vd_step="hi"
        )  # Plotting Id-Vg for all channel lengths in log scale
        self.plot_IdVg_multVds(
            ax[2], fig, "log", channel=0
        )  # Plotting Id-Vg for 100nm in log
        if self.goodTLM:
